...
def score_candidates(
    ...
    *,
    ...
    Uses the sparse exact flow projector. ``terminal_lambda`` credits that many
    extra production-turns to the owner of each planet at the projection horizon's
    end (deployable-tempo fix); 0.0 = stock.
    """
...
def build_target_shortlist(
    ...
    # Optional cheapness-ranked slots: targets whose projected clearing cost dips
    # low within K_eta (e.g. enemy capture in progress ⇒ tiny post-capture
    # garrison) per unit prod. Proximity ranking alone never surfaces these.
    n_cheap = max(0, min(int(getattr(config, "max_cheap_targets", 0)), P))
    if n_cheap > 0:
        pid = int(obs.player_id)
        Kc = max(1, min(int(K_eta), int(garrison_status.ships.shape[-1]) - 1))
        ships_h = garrison_status.ships[:, 1 : Kc + 1].to(prod.dtype)    # [P, Kc]
        owner_h = garrison_status.owner[:, 1 : Kc + 1]
        clear_cost = torch.where(
            owner_h == pid, torch.full_like(ships_h, float("inf")), ships_h,
        )
        min_floor = clear_cost.amin(dim=-1)                              # [P]
        cheap_pref = -min_floor / prod.to(min_floor.dtype).clamp(min=0.25)
        residual = attack_mask.clone()
        residual[atk_idx[atk_exists]] = False
        cheap_idx, cheap_exists = _candidate_indices(cheap_pref, residual, n_cheap)
        atk_idx = torch.cat([atk_idx, cheap_idx], dim=0)
        atk_exists = torch.cat([atk_exists, cheap_exists], dim=0)
...
def _greedy_select(
    *, P, W, device, dtype, score, cand_src, cand_send, cand_angle, cand_eta,
    ...
    target_exists, roi_threshold, taken_init=None,
) -> LaunchEntries:
    ...
    ``taken_init`` ([T] bool, optional) marks shortlist targets already consumed
    by an earlier pass (e.g. the surplus-expand valve re-runs this over leftover
    budget) so a second pass can't pile a redundant fleet onto a planet the first
    pass already captured. None = stock (byte-identical primary pass)."""
    C, L = int(cand_src.shape[0]), int(cand_src.shape[1])
    target_taken = ~target_exists.clone()                                        # [T]
    if taken_init is not None:
        target_taken = target_taken | taken_init
    ...
    # source_budget = leftover ships per planet; target_taken = shortlist targets
    # consumed (feed as taken_init to a follow-up pass to avoid pile-on).
    return entries, source_budget, target_taken
...
def _plan_reinforce(
    *, movement, obs, obs_tensors, garrison_status, cache, leftover,
    original_ships, config, H, fresh_mask=None,
) -> LaunchEntries:
    """Holdability reinforcement of owned planets.

    Sibling of ``_plan_regroup`` but driven by an *absolute* deficit, not a
    pressure gradient: a destination is reinforced only if the do-nothing
    projection loses it within ``reinforce_hold_H`` turns to reachable enemy
    mass, sized to close that deficit, and SKIPPED entirely when the reachable
    reserves cannot save it (``reinforce_unholdable_skip``). Reuses the
    leftover/safe_drain budget caps and the arrival-turn ownership check.
    """
    P = obs.P
    device = obs.device
    dtype = original_ships.dtype
    pid = int(obs.player_id)
    min_send = float(config.min_ships_to_launch)
    Hh = int(config.reinforce_hold_H)
    if Hh <= 0:
        return _empty_entries(device, dtype)

    owner_traj = garrison_status.owner                                   # [P, H+1]
    ships_traj = garrison_status.ships                                   # [P, H+1]
    H_axis = int(ships_traj.shape[-1])
    if bool(getattr(config, "enable_precise_defense", False)):
        # --- precise deficit = the do-nothing projection's ACTUAL loss ---
        # First future turn k<=Hh where an owned planet flips away from us; the
        # enemy's post-combat garrison there is how far we were out-gunned, i.e.
        # the minimal extra ships needed to hold. 0 for planets that don't flip.
        kmax = min(max(Hh, 0), H_axis - 1)
        owned_now = owner_traj[:, 0] == pid                              # [P]
        if kmax >= 1:
            flips = owned_now.view(P, 1) & (owner_traj[:, 1:kmax + 1] != pid)  # [P,kmax]
            any_flip = flips.any(dim=1)                                  # [P]
            kf = torch.where(
                flips,
                torch.arange(1, kmax + 1, device=device).view(1, -1).expand(P, kmax),
                torch.full((P, kmax), H_axis - 1, device=device),
            ).amin(dim=1).clamp(max=H_axis - 1)                         # [P]
            enemy_leftover = ships_traj.gather(1, kf.view(P, 1)).squeeze(1).to(dtype)
            buf = float(getattr(config, "reinforce_deficit_buffer", 1.0))
            deficit = torch.where(
                any_flip, enemy_leftover + buf,
                torch.zeros(P, dtype=dtype, device=device),
            )                                                           # [P]
        else:
            deficit = torch.zeros(P, dtype=dtype, device=device)
    else:
        # --- legacy: enemy reach-mass within Hh, per planet [P] ---
        d0 = cache.cross_dist[0].to(dtype)                                   # [P, P] e->p
        ships_all = obs.ships.to(dtype)
        enemy = obs.alive & (obs.owner_abs >= 0) & (obs.owner_abs != pid)    # [P]
        speeds = fleet_speed(ships_all.clamp(min=1.0))                       # [P]
        tau = d0 / speeds.view(P, 1)                                         # [P_e, P_p]
        eye = torch.eye(P, device=device, dtype=torch.bool)
        arrives = ((tau + 1.0) <= float(Hh)) & enemy.view(P, 1) & ~eye       # [P_e, P_p]
        if bool(config.margin_per_player_max):
            owners = obs.owner_abs.to(torch.long)
            reach = torch.zeros(P, dtype=dtype, device=device)
            for q in owners[enemy].unique().tolist():
                qm = (owners == int(q)).view(P, 1) & arrives
                reach = torch.maximum(reach, (ships_all.view(P, 1) * qm.to(dtype)).sum(dim=0))
        else:
            reach = (ships_all.view(P, 1) * arrives.to(dtype)).sum(dim=0)    # [P_p]
        k_proj = min(max(Hh, 0), H_axis - 1)
        proj_garrison = torch.where(
            owner_traj[:, k_proj] == pid,
            ships_traj[:, k_proj].to(dtype),
            torch.zeros(P, dtype=dtype, device=device),
        )
        deficit = (reach - proj_garrison).clamp(min=0.0)                     # [P]

    dst_mask = obs.owned & obs.alive & (deficit > 0.0)
    comet = is_comet_planet(obs_tensors, P, device)
    if comet is not None:
        dst_mask = dst_mask & ~comet
    ...
    priority = deficit.clone()
    if fresh_mask is not None and float(config.reinforce_fresh_weight) != 1.0:
        priority = torch.where(
            fresh_mask, priority * float(config.reinforce_fresh_weight), priority
        )

    T_cap = max(1, min(int(config.max_regroup_targets_per_source), P))
    dst_idx, dst_exists = _candidate_indices(priority, dst_mask, T_cap)  # [T]
    T = int(dst_idx.shape[0])

    # --- sources: owned with leftover, capped by safe_drain - committed ---
    src_mask = obs.owned & obs.alive & (leftover >= min_send)
    if not bool(src_mask.any()):
        return _empty_entries(device, dtype)
    S_cap = max(1, min(int(config.max_regroup_sources_per_lane), P))
    ...
    committed_s = (orig_s - leftover_s).clamp(min=0.0)
    cap_s = torch.minimum(leftover_s, (drain_s - committed_s).clamp(min=0.0)).floor()
    can_send = src_exists & (cap_s >= min_send)
    if not bool(can_send.any()):
        return _empty_entries(device, dtype)

    # --- aim each source -> dsts at fixed size cap_s, reachable within Hh ---
    reinforce_active = reachable_mask(
        movement, source_idx=src_idx, target_idx=dst_idx,
        fleet_sizes=cap_s.view(S, 1, 1).expand(S, T, 1),
        eta_cap=torch.full((T,), float(Hh), device=device),
    ).squeeze(-1)                                                        # [S, T]
    aim = intercept_angle(
        movement, src_idx.unsqueeze(1), dst_idx.unsqueeze(0),
        cap_s.unsqueeze(1), active=reinforce_active,
    )
    angle = aim["angle"]
    eta = aim["eta"]
    viable = aim["viable"]

    # arrival-turn ownership: dst must still be ours at k = ceil(eta)
    dst_owner = garrison_status.owner[dst_idx.clamp(0, P - 1)]           # [T, H+1]
    k_arr = torch.ceil(eta).clamp(min=0, max=H_axis - 1).to(torch.long)  # [S, T]
    owner_at_k = dst_owner.unsqueeze(0).expand(S, T, H_axis).gather(
        -1, k_arr.unsqueeze(-1)
    ).squeeze(-1)
    still_mine = owner_at_k == pid
    ...
    # each source picks its highest-priority reachable destination
    sc = torch.where(
        valid,
        priority[dst_idx].view(1, T).expand(S, T)
        - float(config.regroup_time_penalty_weight) * eta,
        torch.full((S, T), float("-inf"), dtype=dtype, device=device),
    )
    best_t = _stable_argmax(sc)                                          # [S]
    ...
    best_eta = eta[s_ar, best_t]
    if bool(getattr(config, "enable_precise_defense", False)):
        # cap each send at the destination's precise deficit (floored at
        # min_send) so we hold the planet without over-committing offense ships
        need = deficit[best_dst.clamp(0, P - 1)]
        send_amt = torch.minimum(
            cap_s, torch.maximum(need, torch.full_like(cap_s, min_send))
        ).floor()
        send = torch.where(best_valid, send_amt, torch.zeros_like(cap_s))
    else:
        send = torch.where(best_valid, cap_s, torch.zeros_like(cap_s))

    # unholdable skip: drop sends to any dst whose total arriving < its deficit
    if bool(config.reinforce_unholdable_skip):
        arriving = torch.zeros(P, dtype=dtype, device=device)
        arriving.scatter_add_(0, best_dst.clamp(0, P - 1), send)
        holdable = arriving[best_dst.clamp(0, P - 1)] >= deficit[best_dst.clamp(0, P - 1)]
        best_valid = best_valid & holdable
        send = torch.where(best_valid, send, torch.zeros_like(send))
