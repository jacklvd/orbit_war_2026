...
def _flow_terms_per_planet(
    *,
    ...
    # Terminal continuation value: credit lambda extra production-turns to whoever
    # holds the planet at the projection's final snapshot (owner[..., H]). Fixes
    # horizon-myopia — a capture held to H now reflects its post-horizon production.
    # Gated so lambda==0.0 is byte-identical to the pre-change behaviour.
    if float(terminal_lambda) != 0.0:
        terminal_owner = owner[..., H]                                   # [.., P]
        terminal_amount = prod * alive_pmajor[..., H].to(fdtype) * float(terminal_lambda)  # [.., P]
        terminal_oh = (terminal_owner.unsqueeze(-1) == a_idx)           # [.., P, A]
        produced = produced + terminal_amount.unsqueeze(-1) * terminal_oh.to(fdtype)
